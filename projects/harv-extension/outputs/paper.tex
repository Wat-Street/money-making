...
\noindent
Section 5 is intentionally left as a placeholder in this draft. The available ablation outputs are not yet fully aligned with the ten-asset cross-sectional intraday evaluation used above, so Figures 4 through 6 and Tables 3 and 4 are omitted rather than mixing the main results with older, non-comparable outputs.

\noindent
This omission does not change the headline interpretation of Sections A through D, but it does define the remaining robustness burden. The final Section 5 package should use the same tickers, date range, 5-minute horizon, and train-test alignment as the main intraday panel, and should introduce only already-supported controls rather than new headline models. Its role is to separate three competing explanations for the observed gains: genuinely informative order structure, simple parameter expansion, and accidental feature enrichment.

\noindent
The aligned ablation section should therefore contain three checks. First, a capacity curve should test whether the PM gains saturate once the minimal prime coverage is reached. Second, PM and CP modifiers should test whether extra volatility or jump features improve on the core order-aware construction. Third, randomized and contiguous-random controls should test whether arbitrary feature enrichment can replicate the PM and CP improvements. If the prime-based extensions remain competitive under these controls, the empirical argument becomes stronger; if not, the final claim should be narrowed to the direct out-of-sample performance evidence reported above.
...
\noindent
The present evidence supports a deliberately narrow conclusion: on the ten-asset 5-minute intraday panel, the prime-based extensions improve average forecasting accuracy relative to the HAR-RV baseline, with the strongest and most stable gains coming from CP. The appendix daily mirror shows that this advantage does not automatically transfer to daily sampling. Future work should therefore focus less on adding new model names and more on clarifying when the ordered lag structure is statistically and economically reliable.
